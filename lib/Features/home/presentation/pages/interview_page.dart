  ...
  Widget build(BuildContext context) {
    return BlocConsumer<HomeCubit, HomeStates>(
      ...
      builder: (context, state) {
        ...
        if (cubit.isLoadingInterview) {
          return const Center(
            child: CircularProgressIndicator(color: AppColors.interviewAccent),
          );
        }

        if (cubit.hasErrorInterview) {
          return _ErrorView(onRetry: cubit.getInterviewOnline);
        }

        if (cubit.interviewDataOnlineList.isEmpty) {
          return const NotFoundPage(label: 'No Interview Questions Yet');
        }

        final isSearching = _searchCtrl.text.isNotEmpty;
        final list = isSearching
            ? cubit.searchInterviewListOnline
            : cubit.interviewDataOnlineList;
        ...
        return Column(
          children: [
            SearchTextForm(
              ...
              onChanged: (_) =>
                  cubit.searchForInterviewOnline(_searchCtrl.text),
            ),
            ...
            Expanded(
              child: list.isEmpty
                  ? const NotFoundPage(label: 'No matching questions')
                  : ListView.separated(
